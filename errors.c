...
int type_error(int type_one, int type_two) {
  fprintf(stderr, "TypeError: %s can't be coerced into %s\n", type_name(type_two), type_name(type_one));
  exit(1);
};
...
int no_method_error(char* method, struct ast* ast) {
  switch(ast->node_type) {
    ...
    default : {
      ...
      fprintf(stderr, "NoMethodError: FALTA DISTINGUIR ESTE TIPO");	
...
int undefined_variable_error(char* variable_name) {
  fprintf(stderr, "NameError: undefined local variable or method `%s' for main:Object\n", variable_name);
  exit(1);
};


int wrong_arguments_error(int x, int y) {
  fprintf(stderr, "ArgumentError: wrong number of arguments (%i for %i)\n", x, y);
  exit(1);
};
...
int file_load_error(char* file_name) {
  fprintf(stderr, "LoadError: no such file to load -- %s\n", file_name);
  exit(1);
};

int file_bad_inclusion_line_error(int line_number) {
  fprintf(stderr, "LoadError: %4d bad include line\n", line_number);
  exit(1);
};
